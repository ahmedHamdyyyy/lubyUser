...
class AmenitiesWidget extends StatelessWidget {
  ...
  const AmenitiesWidget({super.key, required this.state});
            ...
            children: [
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: () {
                    Navigator.push(context, MaterialPageRoute(builder: (context) => PlaceOffersView(state: state)));
                  },
...
class TileItemList extends StatelessWidget {
  final HomeState state;
  const TileItemList({super.key, required this.state});
    ...
    return Column(
      children: [
        ...
        (tags.length > 2 && tags[2].isNotEmpty)
            ...
            : const SizedBox.shrink(),
        (tags.length > 3 && tags[3].isNotEmpty)
            ? ListTileItem(icon: 'assets/images/Kitchen.svg', text: tags[3])
            : const SizedBox.shrink(),
        (tags.length > 4 && tags[4].isNotEmpty)
            ? ListTileItem(icon: 'assets/images/king_bed.svg', text: tags[4])
            : const SizedBox.shrink(),
        (tags.length > 5 && tags[5].isNotEmpty)
            ? ListTileItem(icon: 'assets/images/car.svg', text: tags[5])
            : const SizedBox.shrink(),
      ],
    );
  }
