...
import '../../../../../locator.dart';
import '../../../User/screens/propreties/widgets/show_reserve_dialoge.dart';
...
Widget buildHotelReservationCardCurrentReservation({required BuildContext context, required ReservationModel reservation}) {
  final item = reservation.item as PropertyModel;
  return Card(
    color: Colors.white,
    elevation: 0,
      ...
      children: [
                ...
                children: [
                  Row(
                    children: [
                      ...
                      IconButton(
                        onPressed: () => getIt<ReservationsCubit>().removeReservation(reservation.id),
        ...
        ElevatedButton(
          onPressed: () {
            Navigator.push(
              context,
              MaterialPageRoute(builder: (context) => CurrentReservationDetailsScreen(reservation: reservation)),
            );
          },
        ...
        children: [
          Row(
            ...
            children: [
              ...
              ClipRRect(
                borderRadius: BorderRadius.circular(5),
                child: Image.asset(imageUrl, width: 101, height: 101, fit: BoxFit.cover),
              ),
              const SizedBox(width: 12),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 4),
                    Text(location, style: GoogleFonts.poppins(fontSize: 14, color: AppColors.grayTextColor)),
                    const SizedBox(height: 4),
                    RichText(
                      text: TextSpan(
                        style: GoogleFonts.poppins(fontSize: 14, color: AppColors.secondTextColor),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 4),
                    Text(location, style: GoogleFonts.poppins(fontSize: 14, color: Colors.grey.shade600)),
                    const SizedBox(height: 4),
                    RichText(
                      text: TextSpan(
  ...
  return Padding(
    ...
    child: Row(
      children: [
        InkWell(onTap: () => Navigator.pop(context), child: Icon(Icons.arrow_back_ios, color: AppColors.grayColorIcon)),
        const SizedBox(width: 8),
        Text(title, style: GoogleFonts.poppins(color: AppColors.grayTextColor, fontWeight: FontWeight.w500, fontSize: 14)),
      ],
    ),
  );
}
...
Widget reservationTitle(String title) {
  return Text(title, style: GoogleFonts.poppins(fontSize: 16, fontWeight: FontWeight.w600, color: AppColors.primaryColor));
}
          ...
          children: [
            Expanded(
              child: Text(
                ...
                style: GoogleFonts.poppins(color: AppColors.secondTextColor, fontSize: 12, fontWeight: FontWeight.w400),
                overflow: TextOverflow.ellipsis,
              ),
            ),
            if (showActions)
                ...
                children: [
                  IconButton(
                    onPressed: () {
                      showReseverDialoge(
                        context,
                        reservation.item as PropertyModel,
                        TextEditingController(text: reservation.checkInDate.substring(0, 10)),
                        TextEditingController(text: reservation.checkOutDate.substring(0, 10)),
                        TextEditingController(text: reservation.guestNumber.toString()),
                      );
                    },
...
class HostInfo extends StatelessWidget {
  ...
  const HostInfo({super.key, this.showMessageIcon = true});
        ...
        Column(
          ...
          children: [
            Text(
              ...
              style: GoogleFonts.poppins(color: AppColors.secondTextColor, fontWeight: FontWeight.w400, fontSize: 14),
            ),
            Text(
              'Hosted by',
...
Widget viewSummaryRow(BuildContext context, {VoidCallback? onTap}) {
  return Row(
    children: [
      ...
      InkWell(onTap: onTap, child: Icon(Icons.arrow_forward_ios, color: AppColors.grayColorIcon)),
    ],
  );
}
...
class CurrentReservationDetailsContent extends StatelessWidget {
  const CurrentReservationDetailsContent({super.key, required this.reservation});
  final ReservationModel reservation;

  @override
  Widget build(BuildContext context) {
    final item = reservation.item as PropertyModel;
    final nights = DateTime.parse(reservation.checkInDate).difference(DateTime.parse(reservation.checkOutDate)).inDays.abs();
    return SingleChildScrollView(
      padding: EdgeInsets.symmetric(horizontal: 20),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          reservationDetailsAppBar(context, "Current Reservations Details"),
          const SizedBox(height: 22),
          reservationTitle('Reservation Number ${reservation.registrationNumber}'),
          SizedBox(height: 16),
          buildItemCard(
            context: context,
          ...
          SizedBox(height: 8),
          SummaryRow(
            title: '$nights nights × ${reservation.guestNumber} Guests × ${item.pricePerNight}',
            price: '${nights * reservation.guestNumber * item.pricePerNight} SAR',
          ),
          ...
          viewSummaryRow(
            context,
            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => CurretReservationDetailsScreen2())),
          ),
          SizedBox(height: 24),
        ],
      ),
